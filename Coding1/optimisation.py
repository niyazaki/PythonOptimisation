...
for i in range(4):
    ...
    tFor = timeit.Timer('computeFor(numbers{})'.format(
        i), 'from __main__ import computeFor, numbers{}'.format(i))
    secFor = tFor.timeit(repeats) / repeats
    print("{} : {} seconds". format("for", secFor))
    yFor.append(secFor)

    tWhile = timeit.Timer('computeWhile(numbers{})'.format(
        i), 'from __main__ import computeWhile, numbers{}'.format(i))
    secWhile = tWhile.timeit(repeats) / repeats
    print("{} : {} seconds". format("while", secWhile))
    yWhile.append(secWhile)

    tSum = timeit.Timer('computeSum(numbers{})'.format(
        i), 'from __main__ import computeSum, numbers{}'.format(i))
